 ...
 * assert the sequence of confirmed [com.eldercareguardian.data.RiskStatus]
 * values.
 *
...
internal object FallImuPatterns {
    ...
    /**
     * Sanity helper: returns the [com.eldercareguardian.data.RiskStatus] a single
     ...
     */
    fun expectedEngineStatus(imu: List<Float>): com.eldercareguardian.data.RiskStatus {
        if (imu.size < 3) return com.eldercareguardian.data.RiskStatus.Low
        val mag = magnitude(imu[0], imu[1], imu[2])
        return when {
            mag > FALL_SPIKE_THRESHOLD -> com.eldercareguardian.data.RiskStatus.High
            mag < FALL_STILLNESS_THRESHOLD -> com.eldercareguardian.data.RiskStatus.Medium
            else -> com.eldercareguardian.data.RiskStatus.Low
        }
    }
}
